...
(function (JsonSerializer) {
    ...
    function resolveName(key, info, settings) {
        ...
        var type = settings && settings.propertyResolver || 'camelCase';
        if (type === 'camelCase') {
...
(function (JsonConvert) {
    function toJson(model, settings) {
        ...
        var meta = JsonUtils_1.JsonUtils.pickModelMeta(model);
        ...
        for (var key in model) {
            ...
            var property = JsonSerializer_1.JsonSerializer.resolveName(key, propertyInfo, settings);
            var val = JsonSerializer_1.JsonSerializer.resolveValue(model[key], propertyInfo, settings);
            json[property] = val;
        }
        return json;
    }
    ...
    function fromJson(json, Ctor, settings) {
        if (Types_1.Types.isArray(json)) {
            return json.map(function (x) { return fromJson(x, Ctor, settings); });
        }
        var meta = JsonUtils_1.JsonUtils.pickModelMeta(Ctor) || { Type: Ctor, properties: {} };
